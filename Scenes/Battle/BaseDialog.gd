...
#signal run_selected
#  "opmon_selected", "run_selected"
const _signals = ["move_selected", "item_selected"]
...
func update_idle_dialog():
	...
	if battle.player_opmon == null:
		return

	$BigDialog/RichTextLabel.text = tr("BATTLE_BASEDIALOG_FILLER").replace("{opmon}", battle.player_opmon.species.name)
